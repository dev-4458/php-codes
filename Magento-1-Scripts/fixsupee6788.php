...
Usage:  php -f fixSUPEE6788.php -- [options] [recordAffected]
  ...
  module            Module Name to which apply this patch (Mage_Bundle)
...
<adminhtml>
				<args>
					<modules>
						<{$routeTag} after="Mage_Adminhtml">{$module}_Adminhtml</{$routeTag}>
					</modules>
				</args>
			</adminhtml>
XML;
	...
	protected function _moveAdminControllers( $modulePaths, $dryRun=true )
	{
		foreach( $modulePaths as $route => $modulePath ) {
			...
			// Mirasvit requirement
			$cleanRoute			= str_replace(array('admin', 'adminhtml'), '', $cleanRoute);
			...
			// Mirasvit requirements
			$this->_fileReplacePatterns[ 'handle="' . $route . '_adminhtml_' ]	= 'handle="adminhtml_' . $cleanRoute . '_';
			$this->_fileReplacePatterns[ "_redirect('*/adminhtml_" ]			= "_redirect('*/" . $cleanRoute . '_';
			$this->_fileReplacePatterns[ "->getUrl('*/adminhtml_" ]				= "->getUrl('*/" . $cleanRoute . '_';
		}
			...
			foreach( $files as $file => $object ) {
				...
				foreach( $lines as $key => $line ) {
					foreach( $this->_fileReplacePatterns as $pattern => $replacement ) {
						// Mirasvit requirement
						$module = substr($replacement, strpos($replacement, '/') + 1, (strpos($replacement, '\_') - 1));
						if (strpos($pattern, '_redirect') !== false || strpos($pattern, '->getUrl') !== false) {
							...
							if (strpos($line, $pattern) !== false) {
								...
								if ($checkLine !== $key && strpos($pattern, 'getUrl') !== false && strpos($line,
										'getUrl') !== false
								) {
